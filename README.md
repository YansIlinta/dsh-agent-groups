...
它关注的不是“同时叫几个 Agent 回答一次”，而是让不同 Coding Agent 在同一个 Group 中拥有持续存在的身份、运行时会话、任务历史和共享工作上下文。Leader 可以把工作派发给 DSH Agent、OpenAI Codex 或 Anthropic Claude，并在后续继续追问、纠正、复用同一个会话，而不是每个任务都重新开始。
