...
describe('task lifecycle', () => {
  ...
  it('persists the task-dispatch outbox and one-way delivery lease states', async () => {
    const h = makeHarness()
    const group = await seedGroup(h)
    const task = await h.tasks.createTask(group.groupId, { createdBy: 'lead', subject: 'dispatch', description: '…', kind: 'implementation', acceptanceCriteria: ['ok'] })
    const assigned = await h.tasks.assign(group.groupId, task.taskId, 'm1', 'lead', undefined, true)
    expect(assigned.dispatch).toMatchObject({ state: 'pending', sequence: 1, ownerId: 'm1' })
    const leaseId = await h.tasks.beginDispatch(group.groupId, task.taskId, 'm1')
    expect(h.tasks.requireTask(group.groupId, task.taskId).dispatch).toMatchObject({ state: 'dispatching', leaseId })
    await h.tasks.markDispatchDelivered(group.groupId, task.taskId, leaseId)
    expect(h.tasks.requireTask(group.groupId, task.taskId).dispatch).toMatchObject({ state: 'delivered', leaseId })
    await expect(h.tasks.markDispatchAmbiguous(group.groupId, task.taskId, leaseId, 'late failure'))
      .rejects.toMatchObject({ code: 'CONFLICT' })
    const reassignedWithoutDelivery = await h.tasks.assign(group.groupId, task.taskId, 'm2', 'lead')
    expect(reassignedWithoutDelivery.dispatch).toBeUndefined()
  })
  ...
  it('records a new attempt sequence after a task is reopened', async () => {
    ...
    await h.tasks.assign(group.groupId, task.taskId, 'm1', 'lead', undefined, true)
    await h.tasks.claim(group.groupId, task.taskId, 'm1')
    await h.tasks.markFailed(group.groupId, task.taskId, 'm1', 'crashed')
    await h.tasks.reopen(group.groupId, task.taskId, 'lead', 'retry')
    expect(h.tasks.requireTask(group.groupId, task.taskId).dispatch).toBeUndefined()
