...
目标是让一次视频制作从“调用几个一次性 Agent”变成一个可以持续工作的生产团队：Leader 根据当前生产状态拆解任务，不同 Specialist 在各自长期会话中并行推进，新的研究结果可以产生新的素材任务，素材限制也可以反向触发补充研究，而已经形成的脚本、场景、声音、字幕和渲染结果继续留在同一个生产工作区中。

## 当前能力

Create Flow 直接建立在 Agent Groups 的通用编排能力之上：

- **持久生产成员**：Topic Strategist、Researcher、Material Producer、Scriptwriter、Video Producer 都是长期 Group Member，而不是一次性 subagent。相关后续任务可以继续使用同一个 provider session。
...
这使 Create Flow 更接近 manager-led orchestration：**Lead 保留全局生产控制权，Specialist 保留自己的长期上下文，任务图随着工作结果动态展开。**
...
2. **决定下一批工作**：根据当前目标和缺失信息，把大阶段拆成可执行 Task，决定哪些工作可以并行、哪些需要依赖边。
...
- **独立 Specialist 可以并行，相关工作优先复用长期 Session。**
...
最终希望形成的不是一个“自动按八个按钮跑完视频”的 pipeline，而是一个能够持续观察生产状态、拆解工作、调用长期 Agent 团队并组织真实媒体产物的 **agent-native production workspace**。
